...
public class BoardModel extends BakedModelWrapperWithData {
    ...
    public BoardModel(BakedModel originalModel) {
        super(originalModel);
    }
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, Random rand, IModelData extraData) {
        ...
        for (int i = 0; i < quads.size(); i++) {
            ...
            if (shift) {
                ...
                float quadWidth = newQuad.getSprite().getU1() - newQuad.getSprite().getU0();
                int[] vertexData = newQuad.getVertices();

                for (int vertex = 0; vertex < 4; vertex++) {
                    float u = QuadHelper.getU(vertexData, vertex);
                    //Width/2 because the texture is 32x, so one 16x block is half that.
                    QuadHelper.setU(vertexData, vertex, u + ((quadWidth/2) * flip));
                }
